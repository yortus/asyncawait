...
describe('async.iterable.promise(...)', () => {

    var foo = async.iterable.promise ((count: number, accum?: any[]) => {
    ...
    describe('provides an iterator whose next() method', () => {
        ...
        it("preserves the 'this' context of the call", async.cps (() => {
            var foo = { bar: async.iterable.promise (function () { yield_ (this); return 'done'; }) }, baz = {x:7};
            var iter = foo.bar();
        ...
        it('works with await', done => {
            var foo = async.iterable.promise (() => { yield_ (await (Promise.delay(20).then(() => 'blah'))); });
            foo().next()
    ...
    describe('provides an iterator whose forEach() method', () => {
        ...
        it('works with await', done => {
            var foo = async.iterable.promise (() => { yield_ (await (Promise.delay(20).then(() => 'blah'))); }), arr = [];
